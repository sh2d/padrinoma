...
local function apply_manipulation(head, twords)
   ...
   for _,word in ipairs(twords) do
      -- Insert artificial spot after the last letter of the word.
      word.levels[#word.levels] = 1
      -- Replace all round s glyphs not followed by a spot by a long s
      -- glyph.
      for i,n in ipairs(word.nodes) do
         if n.char == 0x73 and not (word.levels[i+1] % 2 == 1) then
            n.char = 0x017f
         end
      end
   end
...
local scan_node_list = padrinoma.create_node_list_scanner('german', '../../patterns/gsub-long-s-neg-de-1901.pat.txt', 0, 0, true)
