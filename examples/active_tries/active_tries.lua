...
Reads-in a list of strings and decomposes them using Liang patterns. Statistical data is collected about the number of active tries per letter position. Diagrams are created for all string lengths as well as the complete list. Strings are read from standard input and must be in the UTF-8 encoding. A pattern file specified via -p must contain pure text patterns in the UTF-8 encoding. Pattern files are searched using the kpse library. Using a boundary letter different to the one used during pattern creation can lead to wrong results.]]
   )
end
